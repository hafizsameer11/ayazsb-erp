...
<div class="erp-panel border border-slate-500 bg-white shadow-md">
    ...
    <form class="space-y-3 p-3" data-yarn-issuance-form data-yarn-line-form data-erp-ajax-save @if($editingTransaction) data-erp-editing="1" @endif
        ...
        <fieldset class="border border-slate-400 bg-[#f4f4f4] p-2">
            ...
            <div class="grid gap-2 md:grid-cols-2 lg:grid-cols-4">
                ...
                @if($formVariant === 'issuance-transfer')
                    ...
                    <label class="erp-field md:col-span-2"><span class="erp-label">From Contract (Issuance)</span>
                        <select class="erp-input" name="source_transaction_id">
                            <option value=""></option>
                            @foreach($yarnIssuanceOptions ?? [] as $issue)
                                <option value="{{ $issue['id'] }}" data-payload="{{ json_encode($issue) }}" @selected((string)old('source_transaction_id',$editingTransaction?->source_transaction_id)===(string)$issue['id'])>{{ $issue['trans_no'] }} — {{ $issue['contract_code'] }} — {{ $issue['grey_quality_no'] }}</option>
                            @endforeach
                        </select>
                    </label>
                    ...
                    <label class="erp-field md:col-span-2"><span class="erp-label">To Contract (Grey Conversion)</span>
                        <select class="erp-input" name="to_grey_conversion_contract_id">
                            <option value=""></option>
                            @foreach($greyConversionContracts as $c)
                                <option value="{{ $c->id }}" data-payload="{{ json_encode(['account_id' => $c->account_id]) }}" @selected((string)old('to_grey_conversion_contract_id',$editingTransaction?->grey_conversion_contract_id)===(string)$c->id)>{{ $c->contract_no }} — {{ $c->account?->name }} — {{ $c->quality?->quality_no }}</option>
                            @endforeach
                        </select>
                    </label>
                @else
                    <label class="erp-field md:col-span-2"><span class="erp-label">Party ID and Name</span>
                        <select class="erp-input" name="account_id" required @if($formVariant === 'issuance-return') @endif>
                            <option value=""></option>
                            @foreach($accountParties as $a)
                    ...
                    @if($formVariant === 'issuance-return')
                        ...
                        <label class="erp-field md:col-span-2"><span class="erp-label">Contract ID</span>
                            <input class="erp-input bg-slate-50" readonly name="meta[contract_display]" value="{{ old('meta.contract_display', $meta['contract_display'] ?? '') }}">
                    ...
                    @else
                        <label class="erp-field md:col-span-2"><span class="erp-label">Contract ID (Grey Conversion)</span>
                            <select class="erp-input" name="grey_conversion_contract_id" required>
                                <option value=""></option>
                                @foreach($greyConversionContracts as $c)
                                    <option value="{{ $c->id }}" data-payload="{{ json_encode(['account_id' => $c->account_id, 'contract_no' => $c->contract_no, 'contract_code' => $c->contract_code, 'contract_date' => $c->contract_date?->format('Y-m-d'), 'grey_quality_no' => $c->quality?->quality_no, 'qty_mtr' => $c->qty_mtr]) }}" @selected((string)old('grey_conversion_contract_id',$editingTransaction?->grey_conversion_contract_id)===(string)$c->id)>{{ $c->quality?->quality_no }} — {{ $c->contract_date?->format('d-m-Y') }} — {{ $c->contract_code ?? $c->contract_no }} — {{ $c->qty_mtr }} Mtr</option>
                                @endforeach
                            </select>
                        </label>
                    @endif
                    ...
                    <fieldset class="border border-slate-400 p-2">
                        ...
                        <div class="grid gap-2" data-yarn-line-row>
                            <label class="erp-field"><span class="erp-label">Yarn Id</span>
                                @if($side === 'from')
                                    <select class="erp-input" name="lines[0][item_id]" data-yarn-item-select @if($formVariant === 'issuance-transfer') readonly @endif>
                                        <option value=""></option>
                                ...
                                @else
                                    <select class="erp-input" name="meta[to_line][item_id]"><option value=""></option>@foreach($items as $item)<option value="{{ $item->id }}">{{ $item->code }}</option>@endforeach</select>
                                @endif
                            </label>
                            <label class="erp-field"><span class="erp-label">Packing Size</span><input class="erp-input bg-slate-50" name="{{ $side === 'from' ? 'lines[0][meta][packing_size]' : 'meta[to_line][packing_size]' }}" data-yarn-packing-size readonly></label>
                            <label class="erp-field"><span class="erp-label">No of Bags</span><input class="erp-input" name="{{ $side === 'from' ? 'lines[0][qty]' : 'meta[to_line][qty]' }}" data-yarn-bags type="number" step="0.0001"></label>
                            <label class="erp-field"><span class="erp-label">No of Cones</span><input class="erp-input" name="{{ $side === 'from' ? 'lines[0][meta][no_of_cones]' : 'meta[to_line][no_of_cones]' }}" data-yarn-cones type="number" step="0.0001"></label>
                            <label class="erp-field"><span class="erp-label">Rate / LBs</span><input class="erp-input" name="{{ $side === 'from' ? 'lines[0][rate]' : 'meta[to_line][rate]' }}" data-yarn-rate type="number" step="0.0001" @if($side==='from') readonly @endif></label>
                            <label class="erp-field"><span class="erp-label">Total LBs</span><input class="erp-input bg-slate-50" name="{{ $side === 'from' ? 'lines[0][weight_lbs]' : 'meta[to_line][weight_lbs]' }}" data-yarn-weight-lbs readonly></label>
                            ...
                            <label class="erp-field"><span class="erp-label">Total Amount</span><input class="erp-input bg-slate-50" name="{{ $side === 'from' ? 'lines[0][amount]' : 'meta[to_line][amount]' }}" data-yarn-amount readonly></label>
                        </div>
                    </fieldset>
                @endforeach
            </div>
                    ...
                    <tbody data-yarn-line-rows>
                        @foreach($lineRows as $i => $line)
                            <tr data-yarn-line-row>
                                <td class="border p-0.5"><select class="erp-input w-full" name="lines[{{ $i }}][item_id]" data-yarn-item-select @if($formVariant==='issuance-return') disabled @endif><option value=""></option>@foreach($items as $item)<option value="{{ $item->id }}" @selected((string)old("lines.$i.item_id",$line?->item_id)===(string)$item->id)>{{ $item->code }} — {{ $item->name }}</option>@endforeach</select></td>
